...
if (!$url) {
    echo json_encode(['error' => 'URL profil wajib diisi.']);
    http_response_code(400);
    exit;
}

if (!preg_match('/^(https?:\/\/)?(www\.)?(skills\.google|cloudskillsboost\.google)\/public_profiles\/[a-f0-9-]+/i', $url)) {
    echo json_encode(['error' => 'URL harus berupa tautan profil publik Google Cloud Skills Boost yang sah.']);
    http_response_code(400);
    exit;
}
...
if ($html === false) {
    echo json_encode(['error' => 'Gagal mengambil data profil publik. Pastikan profil diatur ke publik.']);
...
// Update DB
$stmt = $db->prepare("SELECT id FROM leaderboard_participants WHERE profile_url = ?");
$stmt->execute([$url]);
$existing = $stmt->fetch();

if ($existing) {
    $updateStmt = $db->prepare("UPDATE leaderboard_participants SET name=?, profile_token=?, arcade_count=?, skill_count=?, bonus_points=?, total_points=?, milestone_reached=?, last_checked_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE profile_url=?");
    $updateStmt->execute([$name, $profileToken, $arcadeCount, $skillCount, $bonusPoints, $totalPoints, $milestoneReached, $url]);
} else {
    $insertStmt = $db->prepare("INSERT INTO leaderboard_participants (profile_url, name, profile_token, arcade_count, skill_count, bonus_points, total_points, milestone_reached, last_checked_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)");
    $insertStmt->execute([$url, $name, $profileToken, $arcadeCount, $skillCount, $bonusPoints, $totalPoints, $milestoneReached]);
}
