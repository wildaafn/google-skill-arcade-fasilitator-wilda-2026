    ...
    <div class="mutualan-container">
      ...
      <div>
        <div class="mutualan-tabs">
          <button class="mutual-tab-btn active" id="tab-btn-instagram" onclick="switchMutualTab('instagram')">
            <i class="fa-brands fa-instagram"></i> Instagram ({{ $instagramProfiles->count() }})
          </button>
          <button class="mutual-tab-btn" id="tab-btn-linkedin" onclick="switchMutualTab('linkedin')">
            <i class="fa-brands fa-linkedin"></i> LinkedIn ({{ $linkedinProfiles->count() }})
          </button>
          <button class="mutual-tab-btn" id="tab-btn-github" onclick="switchMutualTab('github')">
            <i class="fa-brands fa-github"></i> GitHub ({{ $githubProfiles->count() }})
          </button>
        </div>
        ...
        <div class="mutual-grid active instagram-theme" id="grid-instagram">
          @forelse($instagramProfiles as $profile)
            <div class="mutual-card @if(str_contains($profile->username, 'wilda')) admin-card @endif">
              <div class="mutual-info">
                <div class="mutual-avatar-icon">
                  <i class="fa-brands fa-instagram"></i>
                </div>
                <div class="mutual-details">
                  <h5>{{ $profile->username }}</h5>
                  @if(str_contains($profile->username, 'wilda'))
              ...
              <a href="{{ $profile->link }}" target="_blank" class="mutual-action-btn" aria-label="Follow {{ $profile->username }}">
        ...
        <div class="mutual-grid linkedin-theme" id="grid-linkedin">
          @forelse($linkedinProfiles as $profile)
            <div class="mutual-card @if(str_contains($profile->username, 'WILDA')) admin-card @endif">
              <div class="mutual-info">
                <div class="mutual-avatar-icon">
              ...
              <a href="{{ $profile->link }}" target="_blank" class="mutual-action-btn" aria-label="Connect {{ $profile->username }}">
                <i class="fa-solid fa-user-plus"></i>
              </a>
            </div>
          @empty
        ...
        <div class="mutual-grid github-theme" id="grid-github">
          @forelse($githubProfiles as $profile)
            <div class="mutual-card @if(str_contains($profile->username, 'wilda')) admin-card @endif">
              <div class="mutual-info">
                <div class="mutual-avatar-icon">
      ...
      <div>
        <div class="mutual-form-card glass">
          ...
          <form id="mutual-form-laravel" style="display: flex; flex-direction: column; gap: 16px;">
            @csrf
            ...
            <button type="submit" id="mutual-submit-btn-laravel" class="btn btn-primary btn-block" style="margin-top: 8px;">
              Kirim Link Profil <i class="fa-solid fa-paper-plane"></i>
            </button>
          </form>

          <!-- Success Message -->
          <div id="mutual-success-laravel" class="hidden" style="margin-top: 16px; padding: 12px; border-radius: var(--radius-sm); background: var(--accent-green-subtle); border: 1px solid rgba(61, 171, 94, 0.25); color: var(--text-primary); font-size: 0.82rem; display: flex; align-items: flex-start; gap: 10px;">
            <i class="fa-solid fa-circle-check text-green" style="margin-top: 3px;"></i>
            <span id="mutual-success-text">Berhasil terkirim! Profil kalian akan segera diverifikasi dan dimasukkan ke list oleh Admin.</span>
          </div>

          <!-- Error Message -->
          <div id="mutual-error-laravel" class="hidden" style="margin-top: 16px; padding: 12px; border-radius: var(--radius-sm); background: var(--accent-red-subtle); border: 1px solid rgba(224, 85, 69, 0.25); color: var(--text-primary); font-size: 0.82rem; display: flex; align-items: flex-start; gap: 10px;">
...
@section('scripts')
<script>
  // AJAX form submission with CSRF and validation for security
  const formLaravel = document.getElementById('mutual-form-laravel');
  const successDiv = document.getElementById('mutual-success-laravel');
  const successText = document.getElementById('mutual-success-text');
  const errorDiv = document.getElementById('mutual-error-laravel');
  const errorText = document.getElementById('mutual-error-text');
  const submitBtn = document.getElementById('mutual-submit-btn-laravel');

  if (formLaravel) {
    formLaravel.addEventListener('submit', (e) => {
      e.preventDefault();
      ...
      .then(async response => {
        ...
        if (response.ok && data.success) {
          ...
          errorText.textContent = data.message || 'Validation failed. Please check inputs.';
